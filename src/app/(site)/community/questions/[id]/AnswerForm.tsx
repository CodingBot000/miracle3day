...
export default function AnswerForm({ questionId, pointsReward, isAuthenticated }: AnswerFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (content.length < 10) {
      toast.success(language === 'ko' ? '답변은 최소 10자 이상 작성해주세요' : 'Answer must be at least 10 characters long');
      return;
    }
    ...
    <>
      <form onSubmit={handleSubmit} className="space-y-4">
        <textarea
          ...
          placeholder={language === 'ko' ? '당신의 경험과 조언을 공유해주세요... (최소 10자)' : 'Share your experience and advice... (Minimum 10 characters)'}
        ...
        <div className="flex items-center justify-between">
          <span className="text-sm text-gray-500">
            {content.length} {language === 'ko' ? '자' : 'characters'} {content.length >= 10 ? '✓' : (language === 'ko' ? '(최소 10자)' : '(Minimum 10 characters)')}
          </span>
          ...
          >
            {loading ? (language === 'ko' ? '등록 중...' : 'Registering...') : (language === 'ko' ? `답변 등록 (+${pointsReward} 포인트)` : `Submit Answer (+${pointsReward} points)`)}
          </button>
        </div>
      </form>
      {loginModal}
