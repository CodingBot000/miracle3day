...
export default function AnswerItem({ answer, index, questionId, currentUserUuid, onAnswerUpdated }: AnswerItemProps) {
  ...
  const handleSaveEdit = async () => {
    if (!editContent.trim()) {
      toast.error(locale === 'ko' ? '답변 내용을 입력해주세요' : 'Please enter your answer content');
      return;
    }

    if (editContent.length < 10) {
      toast.error(locale === 'ko' ? '답변은 최소 10자 이상 작성해주세요' : 'Answer must be at least 10 characters long');
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast.success(locale === 'ko' ? '✏️ 답변이 수정되었습니다!' : '✏️ Answer has been updated!');
        onAnswerUpdated(updatedAnswer);
        setIsEditing(false);
      } else {
        const { error } = await res.json();
        toast.error(error || (locale === 'ko' ? '답변 수정에 실패했습니다' : 'Failed to update answer'));
      }
    } catch (error) {
      console.error('Edit answer error:', error);
      toast.error(locale === 'ko' ? '답변 수정 중 오류가 발생했습니다' : 'Failed to update answer');
    } finally {
      setIsSaving(false);
    }
  };
    ...
    <div className={`border-2 rounded-xl p-5 transition-all ${
      ...
      {answer.is_best && (
        <div className="inline-flex items-center gap-2 bg-yellow-400 text-yellow-900 px-3 py-1.5 rounded-full text-sm font-bold mb-3">
          <span>⭐</span>
          <span>{locale === 'ko' ? '베스트 답변' : 'Best Answer'}</span>
        </div>
      )}
        ...
        <div className="mb-4">
          ...
          <div className="flex items-center justify-between mt-2">
            <span className="text-sm text-gray-500">
              {editContent.length} {locale === 'ko' ? '자' : 'characters'} {editContent.length >= 10 ? '✓' : (locale === 'ko' ? '(최소 10자)' : '(Minimum 10 characters)')}
            </span>
          </div>
        </div>
      ) : (
      ...
      <div className="flex items-center gap-4 pt-4 border-t border-gray-200">
        {isEditing ? (
          <>
            ...
            >
              <span>💾</span>
              <span>{isSaving ? (locale === 'ko' ? '저장 중...' : 'Saving...') : (locale === 'ko' ? '저장' : 'Save')}</span>
            </button>
            <button
          ...
          <>
            ...
            >
              <span>{liked ? '❤️' : '🤍'}</span>
              <span>{locale === 'ko' ? '좋아요' : 'Like'} {likeCount}</span>
            </button>
            ...
            {!answer.is_best && (
              <button className="flex items-center gap-2 px-4 py-2 rounded-lg font-medium bg-gray-100 text-gray-600 hover:bg-gray-200 transition-all">
                <span>🏆</span>
                <span>{locale === 'ko' ? '베스트 선정' : 'Best Selection'}</span>
              </button>
            )}
          </>
        )}
