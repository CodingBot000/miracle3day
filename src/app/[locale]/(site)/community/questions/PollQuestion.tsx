...
export default function PollQuestion({ question }: PollQuestionProps) {
  ...
  const handleVote = async (optionId: number) => {
    ...
    if (optionId === selectedOptionId) {
      toast.info(locale === 'ko' ? '이미 선택한 옵션입니다' : 'You already selected this option');
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (message?.includes('changed')) {
          toast.success(locale === 'ko' ? '🔄 투표가 변경되었습니다!' : '🔄 Vote changed successfully!');
        } else {
          toast.success(
            locale === 'ko'
        ...
        if (alreadySelected) {
          ...
          toast.error(locale === 'ko' ? '이미 투표하셨습니다!' : 'You have already voted on this poll!');
        } else {
          toast.error(error || (locale === 'ko' ? '투표 실패' : 'Failed to vote'));
        }
      }
    } catch (error) {
      // 에러 시 롤백
