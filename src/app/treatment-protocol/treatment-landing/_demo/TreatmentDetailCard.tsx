...
function effectLabel(attr: Treatment["attributes"] | undefined, locale: "ko" | "en") {
  ...
  const parts: string[] = [];
  // Main line
  const note = e.ko || e.en;
  if (note) parts.push(locale === "ko" ? (e.ko || e.en || "") : (e.en || e.ko || ""));
  // Onset & duration
  const onset = t(e.onset_label, locale) ||
    (e.onset_weeks_min != null ? `${e.onset_weeks_min}${e.onset_weeks_max && e.onset_weeks_max !== e.onset_weeks_min ? `–${e.onset_weeks_max}` : ""}${locale === "ko" ? "주" : " weeks"}` : "");
  if (onset) parts.push(locale === "ko" ? `시술 후 ${onset}부터 효과가 보입니다.` : `Effects typically appear after ${onset}.`);

  const dur = (() => {
    if (e.duration_months_min == null && e.duration_months_max == null) return "";
    const min = e.duration_months_min ?? e.duration_months_max;
    const max = e.duration_months_max && e.duration_months_max !== min ? `–${e.duration_months_max}` : "";
