...
// TOTAL_STEPS는 questions.length에 따라 동적으로 계산됨
...
export default function SkincareOnboardingPage() {
  ...
  // 현재 단계 (0: Welcome, 1: Intro, 2-17: Questions, 18: Completion)
  ...
  // 질문 데이터
  const questions = onboardingData.onboarding.steps;
  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      const onboardingData: SkincareOnboardingDTO = {
        id_uuid: userId,
  ...
  const renderStep = () => {
    ...
    if (step >= 2 && step < 2 + totalQuestions) {
      ...
      if (questions[questionIndex].id === 'country') {
        return (
          <CountryRegionStep
