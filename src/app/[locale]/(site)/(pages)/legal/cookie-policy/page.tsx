...
export default function CookiePolicyPage() {
  ...
  const consentLabel = useMemo(() => {
    if (consent === 'accepted') return t[currentLocale].state_accepted;
    if (consent === 'rejected') return t[currentLocale].state_rejected;
    return t[currentLocale].state_none;
  }, [consent, locale]);
  ...
  return (
    <main className="mx-auto max-w-3xl px-4 py-10 text-gray-800 leading-relaxed">
      <h1 className="text-3xl font-bold mb-2">{t[currentLocale].title}</h1>
      <p className="text-sm text-gray-500 mb-8">{t[currentLocale].updated}</p>

      {/* Section 1 */}
      <section className="mb-10">
        <h2 className="text-xl font-semibold text-gray-900 mb-3">{t[currentLocale].s1_title}</h2>
        <p className="text-gray-700">{t[currentLocale].s1_body}</p>
      </section>

      {/* Section 2 */}
      <section className="mb-10">
        <h2 className="text-xl font-semibold text-gray-900 mb-3">{t[currentLocale].s2_title}</h2>
        <div className="overflow-x-auto rounded-lg border border-gray-200 shadow-sm">
          <table className="w-full border-collapse text-sm text-gray-700">
            <thead className="bg-gray-50 text-gray-900">
              <tr className="divide-x divide-gray-200">
                {t[currentLocale].table_headers.map((h) => (
            ...
            <tbody className="divide-y divide-gray-200">
              {t[currentLocale].rows.map((r, idx) => (
      ...
      {/* Section 3 */}
      <section className="mb-10">
        <h2 className="text-xl font-semibold text-gray-900 mb-3">{t[currentLocale].s3_title}</h2>
        <p>{t[currentLocale].s3_body}</p>
      </section>

      {/* Section 4 */}
      <section className="mb-10">
        <h2 className="text-xl font-semibold text-gray-900 mb-3">{t[currentLocale].s4_title}</h2>
        <ul className="list-disc ml-5 text-gray-700 space-y-1">
          {t[currentLocale].s4_list.map((li) => (
            <li key={li}>{li}</li>
          ))}
        </ul>
      </section>
      ...
      <section className="mb-12">
        <h2 className="text-xl font-semibold text-gray-900 mb-3">{t[currentLocale].s5_title}</h2>
        <p>
          {t[currentLocale].s5_body_prefix}
          <Link href="mailto:mimotok.official@gmail.com" className="text-blue-600 underline">
            mimotok.official@gmail.com
          </Link>
          {t[currentLocale].s5_body_suffix}
        </p>
      </section>
      ...
      <section aria-label="cookie-consent-controls" className="space-y-3">
        ...
        <div className="flex flex-wrap gap-2">
          <button
            ...
            aria-label={t[currentLocale].btn_accept}
          >
            {t[currentLocale].btn_accept}
          </button>
          <button
            onClick={handleEssentialOnly}
            className="px-4 py-2 rounded-md bg-gray-100 text-gray-800 hover:bg-gray-200"
            aria-label={t[currentLocale].btn_essential}
          >
            {t[currentLocale].btn_essential}
          </button>
          <button
            onClick={handleWithdraw}
            className="px-4 py-2 rounded-md bg-white border text-gray-800 hover:bg-gray-50"
            aria-label={t[currentLocale].btn_withdraw}
          >
            {t[currentLocale].btn_withdraw}
          </button>
        </div>
        <p className="text-xs text-gray-500 mt-2">{t[currentLocale].note_http_only}</p>
      </section>
    </main>
  );
}
