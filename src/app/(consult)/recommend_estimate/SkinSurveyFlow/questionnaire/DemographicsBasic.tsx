            ...
            <div key={question.id} className="space-y-2">
              ...
              {question.helperText && (
                <p className="text-sm text-gray-500">
                  {getLocalizedText(question.helperText, language)}
                </p>
              )}
              <Select
              ...
              >
                <SelectTrigger className="border-rose-200 focus:border-rose-400" translate="no">
                  <SelectValue placeholder={getLocalizedText(question.helperText, language) || ''} />
                </SelectTrigger>
                <SelectContent translate="no">
                  {question.options?.map((option) => (
                    <SelectItem key={option.value} value={option.value} translate="no">
                      <span translate="yes">{getLocalizedText(option.label, language)}</span>
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
