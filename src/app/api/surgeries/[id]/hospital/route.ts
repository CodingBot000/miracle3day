...
import { TABLE_BANNER_ITEM, TABLE_HOSPITAL } from "@/constants/tables";
...
export async function GET(
  ...
  try {
    ...
    if (!surgeriesIds) {
      return NextResponse.json({ data: [], nextCursor: false });
    }
    ...
    const rows = await q(
      `SELECT * FROM ${TABLE_HOSPITAL}
       WHERE show = true AND id_surgeries && $1::text[]
    ...
    const countRows = await q<{ count: number }>(
      `SELECT COUNT(*)::int AS count FROM ${TABLE_HOSPITAL}
       WHERE show = true AND id_surgeries && $1::text[]`,
      [surgeryIdsText]
    );
