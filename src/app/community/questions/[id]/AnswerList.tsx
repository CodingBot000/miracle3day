  ...
  if (!answersList || answersList.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-gray-500 text-lg">{language === 'ko' ? '아직 답변이 없습니다' : 'No answers yet'}</p>
        <p className="text-gray-400 text-sm mt-2">{language === 'ko' ? '첫 번째 답변을 남겨보세요!' : 'Leave the first answer!'}</p>
      </div>
    );
  }
