    ...
    <div className="space-y-6">
        ...
        <div role="radiogroup" aria-label="Age Range" className="grid grid-cols-2 md:grid-cols-2 gap-3">
          {ageGroupQuestion?.options?.map((option) => (
            <ChoiceCard
              key={option.value}
              mode="single"
              title={getLocalizedText(option.label, language)}
      ...
      <div className="space-y-2">
        ...
        <div>
          <p className="mb-2 text-gray-700 font-medium">
            {getLocalizedText(conuntryOfResidenceQuestion?.helperText, language)}
          </p>
          <NationModal
            ...
            title = {getLocalizedText(conuntryOfResidenceQuestion?.title, language)}
          />
        </div>
      </div>
      ...
      <div className="space-y-2">
        <Label htmlFor="phoneNumber" className="text-gray-700 font-medium">
          {language === 'ko' ? '전화번호 (선택)' : 'Phone Number (Optional)'}
        </Label>
        ...
        <InputPhoneNumber
          ...
          placeholder={language === 'ko' ? '전화번호 입력 (숫자만)' : 'Enter phone number (numbers only)'}
