...
export function recommendTreatments(input: RecommendInputs): RecommendationOutput {
  ...
  log.debug(`[MATCHING] After ethnicity adjustments: ${candidates.length} candidates`);
  ...
  if (ethnicityNote) {
    log.debug("[MATCHING] Ethnicity note:", ethnicityNote);
  }
  ...
  if (climateWarningSummary) {
    log.debug("[MATCHING] Climate warning summary:", climateWarningSummary);
  }
